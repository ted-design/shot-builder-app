...
- **Approved fields:** `hiddenImageIds` and `hiddenSessionIds` (both `string[]`, optional) on casting-board entry documents — `clients/{clientId}/projects/{projectId}/castingBoard/{talentId}` (2026-06-12, casting-board image visibility). Curator-set lists of gallery/casting-session image ids (and whole casting-session ids) hidden from a talent's casting-board card and from public casting shares. Type: `CastingBoardEntry` in `shared/types/index.ts`. No `firestore.rules` change: the existing project-scoped wildcard (`match /{collectionId}/{docId}`) already gates `castingBoard` create/update/delete to admin / producer / warehouse (crew read-only), matching the `canManageCasting` UI gate.
