...
- **Sprint S17 (Export Builder v2) COMPLETE** — PR #378 merged. Fully functional block-based PDF builder with real Firestore data, all 9 block types, PDF generation, drag-and-drop, custom variables, templates. Shot creation improvements (title/description split, gender auto-tags, "01" format). Readiness fixes. CI pool fix.
- **Sprint S15-S16 (UX Overhaul + Quality)** — COMPLETE. PR #376 merged. S15e pending (canvas image editor).
...
- **Next:** S18: Canvas image editor (Fabric.js vs Konva.js). S17 backlog: ImageBlock upload, multi-report per project.
...
| Sprint S15c | Export builder scaffold — block types, templates, variables, document operations, localStorage persistence |
...
| Sprint S16 | Quality & consistency — shared components (ViewModeToggle, SearchBar), interactive table system (5 tables), shoot readiness per-colorway, call sheet typography |
| Sprint S17 | Export Builder v2 — real Firestore data, all 9 blocks functional, PDF generation, drag-and-drop, custom variables. Shot creation: title/description split, gender auto-tags, "01" numbers. Readiness: urgency sole indicator, SKU linkage. CI: forks pool fix |
