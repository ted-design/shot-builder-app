...
import { previewRenumber, renumberShots } from "@/features/shots/lib/shotNumbering"
...
export function RenumberShotsDialog({
  ...
  const { changes, unchangedCount } = useMemo(
    () => (open ? previewRenumber(shots) : { changes: [], unchangedCount: 0 }),
    [open, shots],
  )
  ...
  const handleRenumber = async () => {
    ...
    setBusy(true)
    try {
      const count = await renumberShots(shots, clientId)
