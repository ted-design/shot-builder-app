...
describe("computeAssetMetrics", () => {
  it("returns zero metrics for empty assets array", () => {
  ...
  it("returns correct total and submetrics for assets", () => {
    const assets = [{ id: "1" }, { id: "2" }, { id: "3" }, { id: "4" }, { id: "5" }];
    const result = computeAssetMetrics(assets);
    expect(result.total).toBe(5);
    // Mock implementation: 60% images, 40% docs
    expect(result.subMetrics).toHaveLength(2);
    expect(result.subMetrics[0].label).toBe("images");
    expect(result.subMetrics[1].label).toBe("docs");
  });
});

describe("computeActivityMetrics", () => {
  it("returns zero metrics for empty activities array", () => {
  ...
  it("returns correct total and includes this week metric", () => {
    const activities = [{ id: "1" }, { id: "2" }, { id: "3" }];
    const result = computeActivityMetrics(activities);
    expect(result.total).toBe(3);
    expect(result.subMetrics).toContainEqual(
