...
export default function ImageDiagnosticsPage() {
  ...
  if (!isDev) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-semibold text-slate-800">Image diagnostics</h1>
        <p className="mt-4 text-sm text-slate-600">This utility is only available in development builds.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-6">
      <div className="space-y-2">
        ...
        {state.durationMs != null ? (
          <p className="text-xs text-slate-500">Completed in {formatDuration(state.durationMs)}</p>
        ) : null}
      </div>
      {state.error ? (
      ...
      {state.collections.map((collection) => (
        <div key={collection.key} className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-800">{collection.label}</h2>
          ...
          <div className="space-y-3">
            {collection.docs.length === 0 ? (
              <p className="text-sm text-slate-500">No documents sampled.</p>
            ) : (
              collection.docs.map((doc) => (
                <div key={doc.id} className="rounded border border-slate-100 p-3">
                  <div className="mb-2 text-sm font-semibold text-slate-700">{doc.id}</div>
                  {doc.checks.length === 0 ? (
                    <div className="text-xs text-slate-500">No image-like fields found.</div>
                  ) : (
                      ...
                      {doc.checks.map((check, index) => (
                        ...
                        >
                          ...
                          {check.ok ? (
                            ...
                            <div className="mt-1 text-[11px] text-rose-700">{check.error}</div>
                          )}
                        </li>
                      ))}
                    </ul>
