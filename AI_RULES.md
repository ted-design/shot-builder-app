...
8. **Image/logo assets:** When a logo or image must be visible on both light and dark backgrounds, use `dark:hidden` / `hidden dark:block` to swap between color variants (e.g., `immediate-logo-black.svg` for light, `immediate-logo-white.svg` for dark). The six `immediate-*.svg` files in `public/images/brands/` are the current brand assets — the equivalent `.png` files (`immediate-logo-black.png`, `immediate-logo-white.png`) are retained only for archive/legacy-src references and should not be wired into new vNext components. Never hardcode a single color variant if the container respects theme.
